...
namespace locale::testing {
	...
	struct lang_file_bad : TestWithParam<std::string> {
		...
		static void SetUpTestCase() {
			if constexpr (true) {
				auto lngs_file = fs::fopen(fs::path{ LOCALE_data_path } / "truncated.data", "wb");

				lngs_file = fs::fopen(fs::path{ LOCALE_data_path } / "zero.data", "wb");
				write_lngs_head(lngs_file, 0, 0, 2, 0, 0);

				lngs_file = fs::fopen(fs::path{ LOCALE_data_path } / "file_tag.data", "wb");
				write_lngs_head(lngs_file, langtext_tag, 0, 2, 0, 0);

				lngs_file = fs::fopen(fs::path{ LOCALE_data_path } / "header_2.0.data", "wb");
				write_lngs_head(lngs_file, langtext_tag, hdrtext_tag, 2, 0x0000'0200);

				lngs_file = fs::fopen(fs::path{ LOCALE_data_path } / "header_small.data", "wb");
				write_lngs_head(lngs_file, langtext_tag, hdrtext_tag, 1);

				lngs_file = fs::fopen(fs::path{ LOCALE_data_path } / "header_big.data", "wb");
				write_lngs_head(lngs_file, langtext_tag, hdrtext_tag, 4);

				lngs_file = fs::fopen(fs::path{ LOCALE_data_path } / "no_last.data", "wb");
				write_lngs_head(lngs_file);

				lngs_file = fs::fopen(fs::path{ LOCALE_data_path } / "broken_strs_1.data", "wb");
				write_lngs_head(lngs_file);
				write(lngs_file, string_header{ strstext_tag, 2, 0, 5 });
				write_lngs_last(lngs_file);

				lngs_file = fs::fopen(fs::path{ LOCALE_data_path } / "broken_strs_2.data", "wb");
				write_lngs_head(lngs_file);
				write(lngs_file, strstext_tag);
				write(lngs_file, string_header{ strstext_tag, 2, 1, 4 });
				write_lngs_last(lngs_file);

				lngs_file = fs::fopen(fs::path{ LOCALE_data_path } / "broken_attr_1.data", "wb");
				write_lngs_head(lngs_file);
				...
				lngs_file = fs::fopen(fs::path{ LOCALE_data_path } / "broken_attr_2.data", "wb");
				write_lngs_head(lngs_file);
				...
				lngs_file = fs::fopen(fs::path{ LOCALE_data_path } / "broken_keys_1.data", "wb");
				write_lngs_head(lngs_file);
				write(lngs_file, string_header{ keystext_tag, 2, 1, 4 });
				write_lngs_last(lngs_file);

				lngs_file = fs::fopen(fs::path{ LOCALE_data_path } / "broken_keys_2.data", "wb");
				write_lngs_head(lngs_file);
				write(lngs_file, string_header{ attrtext_tag, 5, 1, 7 });
	...
	TEST_P(lang_file_bad, load) {
		...
		auto bytes = fs::fopen(fs::path{ LOCALE_data_path } / path, "rb").read();
