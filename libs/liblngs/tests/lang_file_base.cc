...
namespace lngs::testing {
	...
	TEST_P(lang_file_base, load) {
		...
		for (auto const& str : defs.strings) {
			...
			EXPECT_EQ(
			    expected[0],
			    file.get_string(static_cast<lang_file::identifier>(str.id)));
	...
	TEST_P(lang_file_base, by_keys) {
		...
		if (with_keys) {
			for (auto const& str : defs.strings) {
				auto expected = split_view(str.value, "\0"sv);
				auto id =
				    static_cast<lang_file::identifier>(file.find_key(str.key));
			...
			for (auto const& str : defs.strings) {
				...
				if (!expected[0].empty()) {
					EXPECT_NE(expected[0],
					          file.get_string(
					              static_cast<lang_file::identifier>(id)));
				} else {
	...
	TEST_P(lang_file_base, outside) {
		...
		static constexpr const auto max_id = static_cast<lang_file::identifier>(
		    std::numeric_limits<
		        std::underlying_type_t<lang_file::identifier>>::max());
		...
		for (auto const& str : defs.strings) {
			...
			if (expected.size() > 1) {
				for (intmax_t count = -100; count < 100; ++count) {
					...
					if (expected_plural < 0 ||
					    static_cast<size_t>(expected_plural) >=
					        expected.size()) {
						GTEST_FAIL()
						    << "Mapping for " << count
						    << " outside of tested range: " << expected_plural
						    << " vs. " << str.value;
					}
	...
	static const auto attrz_broken =
	    helper::attrs_t{}.map([](intmax_t) -> intmax_t { return 0; });
