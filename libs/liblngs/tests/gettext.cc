...
namespace lngs::testing {
	...
	public:
		...
		static void SetUpTestCase() {
			if constexpr (false) {
				auto mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "truncated.mo", "wb");
				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "zero.mo", "wb");
				write_mo_head(mo_file, 0, 0, 0, 0, 0, reverse);

				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "empty_BE.mo", "wb");
				write_mo_head(mo_file, 0, 28, 28, 0, 28, reverse);
				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "empty_LE.mo", "wb");
				write_mo_head(mo_file, 0, 28, 28, 0, 28, direct);

				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "empty_rev_nonzero_BE.mo", "wb");
				write_mo_head(mo_file, 0, 28, 28, 0, 28, reverse, 0x11223344);
				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "empty_rev_nonzero_LE.mo", "wb");
				write_mo_head(mo_file, 0, 28, 28, 0, 28, direct, 0x11223344);

				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "ab_BE.mo", "wb");
				...
				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "ab_LE.mo", "wb");
				...
				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "not_asciiz.mo", "wb");
				write_mo_head(mo_file, 1, 28, 36, 0, 44, reverse);
				be(mo_file, (uint32_t)1);
				be(mo_file, (uint32_t)44);
				be(mo_file, (uint32_t)1);
				...
				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "string_missing.mo", "wb");
				write_mo_head(mo_file, 1, 28, 36, 0, 44, reverse);
				be(mo_file, (uint32_t)1);
				be(mo_file, (uint32_t)44);
				be(mo_file, (uint32_t)1);
				...
				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "within_hash.mo", "wb");
				write_mo_head(mo_file, 1, 28, 36, 2, 44, reverse);
				be(mo_file, (uint32_t)1);
				be(mo_file, (uint32_t)44);

				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "over_the_top.mo", "wb");
				write_mo_head(mo_file, 1, 28, 36, 0, 44, reverse);
				be(mo_file, (uint32_t)1);
				be(mo_file, (uint32_t)44);

				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "no_space_for_0.mo", "wb");
				write_mo_head(mo_file, 1, 28, 36, 0, 44, reverse);
				be(mo_file, (uint32_t)1);
				be(mo_file, (uint32_t)44);
				be(mo_file, (uint32_t)1);
				...
				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "no_space_for_strings_1.mo", "wb");
				...
				mo_file = fs::fopen(fs::path{ LOCALE_data_path } / "no_space_for_strings_2.mo", "wb");
	...
	TEST_P(gettext, text) {
		...
		auto mo = fs::path{ LOCALE_data_path } / filename;
	...
	TEST_P(gtt_attr_extr, parse) {
		...
		auto result = lngs::attributes(attrs);
...
#define EXPECT_ATTR(NAME, var) \
			...
			case EXPECT_ATTR(locale::ATTR_CULTURE, culture);
			case EXPECT_ATTR(locale::ATTR_LANGUAGE, language);
			case EXPECT_ATTR(locale::ATTR_PLURALS, plurals);
			default:
				GTEST_FAIL()
	...
	TEST_P(gtt_extract, translations) {
		...
		auto actual = lngs::translations(gtt, strings, warp == Result::Warped, verbose == Reporting::Verbose, src, diag);
	...
	TEST_P(mo_fix, attributes) {
		...
		file actual;
		if (attr_culture)
			actual.attrs.emplace_back(locale::ATTR_CULTURE, *attr_culture);
	...
	static const mo_fix_param attrs[] = {
		{
			{}, {}, {}, true, { warning << lng::ERR_MSGS_ATTR_LANG_MISSING }
		},
		{
			"cy", {}, { {locale::ATTR_CULTURE, "cy"}, {locale::ATTR_LANGUAGE, "\xC5\xB4\xC3\xAA\xC4\xBA\xC5\x9F\xC4\xA5"} }
		},
		{
			"cy", "no-such.txt", { {locale::ATTR_CULTURE, "cy"} }, false, { location{2}[severity::error] << lng::ERR_FILE_NOT_FOUND }
		},
		{
			"cy", "languages.txt", { {locale::ATTR_CULTURE, "cy"}, {locale::ATTR_LANGUAGE, "Cymraeg"} }
		},
		{
			"pl", "languages.txt", { {locale::ATTR_CULTURE, "pl"}, {locale::ATTR_LANGUAGE, "polski"} }
		},
		{
			"pl-PL", "languages.txt", {
				{locale::ATTR_CULTURE, "pl-PL"},
				{locale::ATTR_LANGUAGE, "P\xC3\xB4\xC4\xBA\xC3\xAF\xC5\x9F\xC4\xA5 (P\xC3\xB4\xC4\xBA\xC8\xA7\xC3\xB1\xC4\x91)"}
			}, true, { location{2}[severity::warning] << arg(lng::ERR_LOCALE_MISSING, "pl-PL") }
		},
		{
			"de-AT", "languages.txt", {
				{locale::ATTR_CULTURE, "de-AT"},
				{locale::ATTR_LANGUAGE, "Deutch (\xC3\x96sterreich)"}
			}
		},
		{
			"de", "languages.txt", {
				{locale::ATTR_CULTURE, "de"},
				{locale::ATTR_LANGUAGE, "\xC4\xA0\xC3\xAA\xC8\x93m\xC8\xA7\xC3\xB1"}
			}, true, { location{2}[severity::warning] << arg(lng::ERR_LOCALE_MISSING, "de") }
		},
		{
			"da", "languages.txt", { {locale::ATTR_CULTURE, "da"}, {locale::ATTR_LANGUAGE, "dansk"} }
		},
		{
			"en", "languages.txt", {
				{locale::ATTR_CULTURE, "en"},
				{locale::ATTR_LANGUAGE, "\xC8\x84\xC3\xB1\xC4\x9F\xC4\xBA\xC3\xAF\xC5\x9F\xC4\xA5"}
			}, true, { location{2}[severity::warning] << arg(lng::ERR_LOCALE_MISSING, "en") }
		},
		{
			"noniso", "languages.txt", {
				{locale::ATTR_CULTURE, "noniso"},
			}, true, { location{2}[severity::warning] << arg(lng::ERR_LOCALE_MISSING, "noniso") }
		},
		{
			"noniso-NONISO", "languages.txt", {
				{locale::ATTR_CULTURE, "noniso-NONISO"},
			}, true, { location{2}[severity::warning] << arg(lng::ERR_LOCALE_MISSING, "noniso-NONISO") }
		},
		{
			"en-NONISO", "languages.txt", {
				{locale::ATTR_CULTURE, "en-NONISO"},
				{locale::ATTR_LANGUAGE, "\xC8\x84\xC3\xB1\xC4\x9F\xC4\xBA\xC3\xAF\xC5\x9F\xC4\xA5 "
		...
		},
		{
			"en-Zzzz-US", "languages.txt", {
				{locale::ATTR_CULTURE, "en-Zzzz-US"},
				{locale::ATTR_LANGUAGE, "\xC8\x84\xC3\xB1\xC4\x9F\xC4\xBA\xC3\xAF\xC5\x9F\xC4\xA5 "
		...
		},
		{
			"en-Zzzz-US-NONISO", "languages.txt", {
				{locale::ATTR_CULTURE, "en-Zzzz-US-NONISO"},
				{locale::ATTR_LANGUAGE, "\xC8\x84\xC3\xB1\xC4\x9F\xC4\xBA\xC3\xAF\xC5\x9F\xC4\xA5 "
										"(\xC3\x99\xC3\xB1\xC3\xAF\xC5\xA7\xC3\xAA\xC4\x91 "
										"\xC5\x9E\xC5\xA7\xC8\xA7\xC5\xA7\xC3\xAA\xC5\x9F)"}
		...
		},
		{
			"en-Zzzz", "languages.txt", {
				{locale::ATTR_CULTURE, "en-Zzzz"},
				{locale::ATTR_LANGUAGE, "\xC8\x84\xC3\xB1\xC4\x9F\xC4\xBA\xC3\xAF\xC5\x9F\xC4\xA5"}
			}, true, { location{2}[severity::warning] << arg(lng::ERR_LOCALE_MISSING, "en-Zzzz") }
		},
	};

	INSTANTIATE_TEST_CASE_P(attrs, mo_fix, ValuesIn(attrs));
}
