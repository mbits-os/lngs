...
namespace lngs::app::testing {
	...
	TEST_P(gtt_attr_extr, parse) {
		...
		size_t expected_attrs =
		    (culture ? 1u : 0u) + (language ? 1u : 0u) + (plurals ? 1u : 0u);
...
#define EXPECT_ATTR(NAME, var)                                      \
	...
	if (var) {                                                      \
		EXPECT_EQ(*var, actual.value);                              \
	}                                                               \
	break

				case EXPECT_ATTR(ATTR_CULTURE, culture);
				    case EXPECT_ATTR(ATTR_LANGUAGE, language);
				    case EXPECT_ATTR(ATTR_PLURALS, plurals); default:
					GTEST_FAIL() << " Unknown key: " << actual.key.id << "\n"
	...
	TEST_P(gtt_extract, translations) {
		...
		auto actual =
		    app::translations(gtt, strings, warp == Result::Warped,
		                      verbose == Reporting::Verbose, src, diag);
	...
	struct mo_load : public TestWithDiagnostics<TestWithParam<mo_load_param>> {
		void ExpectEq(const std::vector<tr_string>& expected,
	...
	TEST_P(mo_fix, attributes) {
		auto [attr_culture, ll_cc, expected, expected_result, msgs] =
		    GetParam();
	...
	static const mo_fix_param attrs_tests[] = {
	    ...
	    {"cy",
	     {},
	     {{ATTR_CULTURE, "cy"},
	      {ATTR_LANGUAGE, "\xC5\xB4\xC3\xAA\xC4\xBA\xC5\x9F\xC4\xA5"}}},
	    {"cy",
	     "no-such.txt",
	     {{ATTR_CULTURE, "cy"}},
	     false,
	     {location{2}[severity::error] << lng::ERR_FILE_NOT_FOUND}},
	    {"cy",
	     "languages.txt",
	     {{ATTR_CULTURE, "cy"}, {ATTR_LANGUAGE, "Cymraeg"}}},
	    {"pl",
	     "languages.txt",
	     {{ATTR_CULTURE, "pl"}, {ATTR_LANGUAGE, "polski"}}},
	    {"pl-PL",
	     "languages.txt",
	     {{ATTR_CULTURE, "pl-PL"},
	      {ATTR_LANGUAGE,
	       "P\xC3\xB4\xC4\xBA\xC3\xAF\xC5\x9F\xC4\xA5 "
