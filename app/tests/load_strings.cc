		...
		static void fix_datapath(argumented_string& msg) {
			if (std::holds_alternative<std::string>(msg.value)) {
				...
				if (val.compare(0, 11, "$DATA_PATH/") == 0) {
					val = (TESTING_data_path / val.substr(11)).string();
				}
			}
	...
	static const compilation_result bad[] = {
	    {{},
	     {pos(1, 1)[severity::error]
	      << arg(lng::ERR_EXPECTED, "`['", lng::ERR_EXPECTED_GOT_EOF)}},
	    {R"([] strings {})",
	     {pos(1, 2)[severity::error]
	      << arg(lng::ERR_REQ_ATTR_MISSING, "serial")}},
	    {R"(strings {})",
	     {pos(1, 1)[severity::error]
	      << arg(lng::ERR_REQ_ATTR_MISSING, "serial")}},
	    ...
	    {R"([serial(abc)] strings {})",
	     {pos(1, 9)[severity::error]
	      << arg(lng::ERR_EXPECTED, lng::ERR_EXPECTED_NUMBER, "`abc'")}},
	    {R"([serial(0)] strings {)",
	    ...
	    {R"([serial(0)] strings { [help()] ID = "value"; })",
	     {pos(1, 29)[severity::error]
	      << arg(lng::ERR_EXPECTED, lng::ERR_EXPECTED_STRING, "`)'")},
	     false,
	     serial_0},
	    {R"([serial(0)] strings { [help] ID = "value"; })",
	    ...
	    {R"(%)",
	     {pos(1, 1)[severity::error]
	      << arg(lng::ERR_EXPECTED, "`['", lng::ERR_EXPECTED_GOT_UNRECOGNIZED)},
	     false,
	     test_strings(0, -1).make()},
	    {R"([string)",
	     {pos(1, 8)[severity::error] << arg(lng::ERR_EXPECTED,
	                                        lng::ERR_EXPECTED_ID,
	    ...
	    {R"("string")",
	     {pos(1, 1)[severity::error]
	      << arg(lng::ERR_EXPECTED, "`['", lng::ERR_EXPECTED_GOT_STRING)},
	     false,
	     test_strings(0, -1).make()},
	    {R"(123)",
	     {pos(1, 1)[severity::error]
	      << arg(lng::ERR_EXPECTED, "`['", lng::ERR_EXPECTED_GOT_NUMBER)},
	     false,
	     test_strings(0, -1).make()},
	    {R"([)",
	     {pos(1, 2)[severity::error] << arg(lng::ERR_EXPECTED,
	                                        lng::ERR_EXPECTED_ID,
	                                        lng::ERR_EXPECTED_GOT_EOF)},
	     false,
	     test_strings(0, -1).make()},
	    ...
	    {R"([dummy(value)])",
	     {pos(1, 8)[severity::error]
	      << arg(lng::ERR_EXPECTED, lng::ERR_EXPECTED_STRING, "`value'")},
	     false,
	     test_strings(0, -1).make()},
	    {R"([serial(0)] ropes {})",
	     {pos(1, 13)[severity::error]
	      << arg(lng::ERR_EXPECTED, "`strings'", "`ropes'")},
	     false,
	     serial_0},
	    {R"([serial(0)] strings ())",
	    ...
	    {R"([serial(0)] strings { [help("help"), id(0), dummy("value") dummy2})",
	     {pos(1, 60)[severity::error]
	      << arg(lng::ERR_EXPECTED, "`,'", "`dummy2'")},
	     false,
	     serial_0},
	    {R"([serial(0)] strings { [help("help"), id(0)]; })",
	     {pos(1, 44)[severity::error]
	      << arg(lng::ERR_EXPECTED, lng::ERR_EXPECTED_ID, "`;'")},
	     false,
	     serial_0},
	...
	static const compilation_result files[] = {
	    {"no-such.idl",
	     {pos(0, 0)[severity::error]
	      << arg(lng::ERR_FILE_MISSING, "$DATA_PATH/no-such.idl"s)},
	     false,
	     {},
	     false},
	    {"no-such.idl",
	    ...
	    {"bad.idl",
	     {pos(0, 0)[severity::verbose] << "$DATA_PATH/bad.idl"s,
	      pos(1, 2)[severity::error]
	          << arg(lng::ERR_REQ_ATTR_MISSING, "serial"),
	      pos(0, 0)[severity::error]
	          << arg(lng::ERR_NOT_STRINGS_FILE, "$DATA_PATH/bad.idl")},
	     false,
	     {},
	     true},
	    {"empty.idl",
