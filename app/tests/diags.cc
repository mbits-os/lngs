...
namespace lngs::app::testing {
	...
	TEST_F(diag_write, lines) {
		...
		for (unsigned ln = 2 * static_cast<unsigned>(std::size(raven)); ln > 0;
		     --ln) {
	...
	diagnostic conv(diagnostics& diag, const expected_diagnostic& exp) {
		const auto loc =
		    diag.source(exp.filename).position(exp.line, exp.column);
		auto end =
		    exp.column_end ? loc.moved(loc.line, exp.column_end) : location{};
	...
	TEST_P(diag_write, print) {
		...
		auto use_tr = [use_alt = value.use_alt_tr](strings& normal,
		                                           strings& alt) -> strings& {
		...
		std::string expected;
		expected.reserve(std::accumulate(
		    begin(exp_str), end(exp_str), 0u,
	...
	void output(std::string& expected,
	            It& curr,
	            It end,
	            const expected_diagnostic& value,
	            const std::string_view (&text)[length]) {
	...
	TEST_P(diag_write, print_with_source) {
		...
		diagnostics diag;
		{
			...
			contents.reserve(std::accumulate(std::begin(text), std::end(text),
			                                 0u,
			                                 [](size_t acc, const auto& view) {
	...
	TEST_P(diag_write, has_errors) {
		...
		ASSERT_EQ(expected, diag.has_errors())
		    << UnexpectedDiags::name(value.sev);
	}
	...
	static const diagnostic_write path_message[] = {
	    ...
	    {{"path", 1, 11, severity::note, lng::ERR_ID_MISSING_HINT,
	      link_type::gcc, false},
	     {"path:1:11: note: before finalizing a value, use `id(-1)'\n"}},
	    {{"path", 2, 12, severity::note, lng::ERR_ID_MISSING_HINT},
	     {"path:2:12: nnn: ERR_ID_MISSING_HINT\n"}},
	    {{"path", 3, 13, severity::error, str(lng::ERR_EXPECTED, "A", "B"),
	      link_type::gcc, false},
	     {"path:3:13: error: expected A, got B\n"}},
	    {{"path", 4, 14, severity::error, str(lng::ERR_EXPECTED, "A", "B")},
	     {"path:4:14: eee: ERR_EXPECTED(A, B)\n"}},
	};
	...
	struct diag_ne
	    : TestWithParam<std::pair<argumented_string, argumented_string>> {};
	...
	static const std::pair<argumented_string, argumented_string> neq[] = {
	    ...
	    {arg(lng::ERR_FILE_MISSING, lng::ERR_EXPECTED_GOT_EOF),
	     arg(lng::ERR_FILE_MISSING, "EOF"s)}};
